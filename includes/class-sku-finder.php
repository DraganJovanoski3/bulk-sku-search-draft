...
class BSSD_SKU_Finder {
	...
	public static function search( $skus ) {
		...
		foreach ( $skus as $sku ) {
			foreach ( BSSD_SKU_Parser::sku_lookup_variants( $sku ) as $variant ) {
				$lookup_values[] = BSSD_SKU_Parser::normalize_sku( $variant );
			}
		}
		...
		foreach ( $rows as $row ) {
			$normalized = BSSD_SKU_Parser::normalize_sku( $row['sku'] ?? '' );
			if ( '' === $normalized ) {
				continue;
			}
		...
		foreach ( $skus as $input_sku ) {
			...
			foreach ( $matches_by_normalized as $matches ) {
				foreach ( $matches as $match ) {
					if ( ! BSSD_SKU_Parser::skus_equivalent( $input_sku, $match['sku'] ?? '' ) ) {
						continue;
					}

					$row_key = (int) ( $match['product_id'] ?? 0 ) . ':' . BSSD_SKU_Parser::normalize_sku( $input_sku );
