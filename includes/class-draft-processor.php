		...
		foreach ( $product_ids as $product_id ) {
			...
			if ( ! $product ) {
				...
				$result['errors'][] = sprintf(
					...
					__( 'Product #%d not found.', 'bulk-sku-search-draft' ),
					$product_id
				);
				continue;
			}
			...
			if ( $save_id && ! is_wp_error( $save_id ) ) {
				...
				++$result['failed'];
				$message = is_wp_error( $save_id ) ? $save_id->get_error_message() : __( 'Unknown error.', 'bulk-sku-search-draft' );
				$result['errors'][] = sprintf(
					/* translators: 1: product ID, 2: error message */
					__( 'Product #%1$d: %2$s', 'bulk-sku-search-draft' ),
					$product_id,
					$message
				);
			}
