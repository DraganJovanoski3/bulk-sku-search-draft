...
class BSSD_SKU_Parser {
	...
	public static function parse( $raw ) {
		...
		if ( '' === trim( $raw ) ) {
			return new WP_Error(
				'bssd_empty_input',
				__( 'Please enter at least one SKU.', 'bulk-sku-search-draft' )
			);
		}
		...
		if ( ! is_array( $lines ) ) {
			return new WP_Error(
				'bssd_parse_failed',
				__( 'Could not parse the SKU input.', 'bulk-sku-search-draft' )
			);
		}
		...
		if ( empty( $skus ) ) {
			return new WP_Error(
				'bssd_no_valid_skus',
				__( 'No valid SKUs found in the input.', 'bulk-sku-search-draft' )
			);
		}

		if ( count( $skus ) > BSSD_MAX_SKUS ) {
			return new WP_Error(
				'bssd_too_many_skus',
				sprintf(
					/* translators: %d: maximum SKU count */
					__( 'Too many SKUs. Maximum allowed is %d.', 'bulk-sku-search-draft' ),
					(int) BSSD_MAX_SKUS
				)
			);
		}
